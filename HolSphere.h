...
class HolSphere :public Hittable
{
public:
	Vector centre;
	float radius;
	...
	HolSphere() {}
	HolSphere(Vector _centre, float _radius, Material* _mat)
	{
...
bool HolSphere::intersect(Ray r, float& t)
{
	float t0, t1;
...
Vector HolSphere::getNormal(Vector hitpos)
{
	Vector sn = -((hitpos - centre) / radius).unitv();
	return sn;
}
/*Material HolSphere :: getMaterial(Vector hitpos)
{
	return *mat;
}*/
Vector HolSphere::getTex(Ray ray, Vector hitpos)
{
